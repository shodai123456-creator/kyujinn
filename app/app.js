...
function blankState() { return { profile: { ...defaultProfile }, weights: { ...defaultWeights }, jobs: demoJobs.map(job => ({ ...job, state: 'new' })), notifications: { morningEnabled: true, morningHour: 7, instantEnabled: true, threshold: 80, minimumConfidence: 80 }, sources: [], recruitment: { sources: [], postings: [], crawlHistory: [] } }; }
...
function showSources() { const list = document.querySelector('#sourceList'); const sources = state.recruitment?.sources || []; list.innerHTML = sources.length ? sources.map(source => `<div class="reason ${source.approvalStatus === 'approved' ? '' : 'negative'}"><strong>${esc(source.companyName)}</strong> / ${esc(source.sourceName)} — ${esc(source.sourceType)}・${esc(source.adapterType)}・${source.enabled ? '有効' : '無効'}・${esc(source.approvalStatus)}${source.entryUrl ? ` <a href="${esc(source.entryUrl)}" target="_blank" rel="noreferrer">取得先</a>` : ''}</div>`).join('') : '<div class="muted">求人ソースはありません。</div>'; }
...
document.querySelector('#sourceForm').addEventListener('submit', async event => { event.preventDefault(); const form = Object.fromEntries(new FormData(event.currentTarget)); const source = { ...form, requiresJavaScript: form.requiresJavaScript === 'true' ? true : form.requiresJavaScript, crawlIntervalHours: Number(form.crawlIntervalHours) || 24, humanReviewed: true }; try { const response = await fetch('/api/recruitment/sources', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(source) }); if (!response.ok) throw new Error(); const added = await response.json(); state.recruitment ||= { sources: [], postings: [], crawlHistory: [] }; state.recruitment.sources.push(added); persistLocal(); showSources(); event.currentTarget.reset(); toast('保留中の求人ソースを追加しました。承認前には取得されません。'); } catch { toast('求人ソースの追加に失敗しました'); } });
document.querySelector('#collectFeeds').addEventListener('click', async () => { try { const response = await fetch('/api/sources/collect', { method: 'POST' }); if (!response.ok) throw new Error(); const collected = await response.json(); if (validState(collected)) { state = { ...state, ...collected }; persistLocal(); render(); showSources(); } toast('承認済み求人ソースを確認しました'); } catch { toast('求人ソース確認に失敗しました。承認状態・URL・ネットワークを確認してください'); } });
...
document.querySelector('#loginForm').addEventListener('submit', async event => { event.preventDefault(); const status = document.querySelector('#loginStatus'); status.textContent = 'ログイン中…'; try { const password = new FormData(event.currentTarget).get('password'); const response = await fetch('/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password }) }); if (!response.ok) throw new Error('パスワードを確認してください'); event.currentTarget.reset(); await boot(); } catch (error) { status.textContent = error.message; } });
document.querySelector('#logout').addEventListener('click', async () => { await fetch('/api/auth/logout', { method: 'POST' }); document.querySelector('#appMain').hidden = true; document.querySelector('#loginCard').hidden = false; document.querySelector('#logout').hidden = true; });
async function boot() { try { const auth = await (await fetch('/api/auth/status')).json(); if (auth.required && !auth.authenticated) { document.querySelector('#loginCard').hidden = false; document.querySelector('#appMain').hidden = true; return; } document.querySelector('#loginCard').hidden = true; document.querySelector('#appMain').hidden = false; document.querySelector('#logout').hidden = !auth.required; showProfile(); showWeights(); showNotifications(); showSources(); render(); await syncFromServer(); } catch { document.querySelector('#appMain').hidden = false; showProfile(); showWeights(); showNotifications(); showSources(); render(); } }
boot();
